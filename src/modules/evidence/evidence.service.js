...
async function addEvidence(activityId, file, user) {
    ...
    await pool.request()
        ...
        .query(`
            INSERT INTO trace.ActivityEvidence (ActivityId, FilePath, FileType)
...
async function deleteEvidence(evidenceId, user) {
    ...
    const r = await pool.request()
        ...
        .query(`
            SELECT 
                ...
                a.TeacherUserId,
                a.Status
            FROM trace.ActivityEvidence e
            JOIN trace.Activities a ON a.Id = e.ActivityId
            WHERE e.Id = @Id
        `);
    ...
    await pool.request()
        .input('Id', sql.UniqueIdentifier, evidenceId)
        .query(`
            DELETE FROM trace.ActivityEvidence 
            WHERE Id = @Id
        `);
